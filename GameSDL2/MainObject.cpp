...
void MainObject::HandleInputAction(SDL_Event events, Mix_Chunk* bullet_sound[2])
{
	...
	else if (events.type == SDL_KEYUP)
	{
		switch (events.key.keysym.sym)
		{
		case SDLK_UP: y_val_ += HEIGHT_MAIN_OBJECT / 4; 
			break;
		case SDLK_DOWN: y_val_ -= HEIGHT_MAIN_OBJECT / 4;
			break;
		case SDLK_LEFT: x_val_ += WIDTH_MAIN_OBJECT / 4;
			break;
		case SDLK_RIGHT: x_val_ -= WIDTH_MAIN_OBJECT / 4;
			break;
		}
	}
	else if (events.type == SDL_MOUSEBUTTONDOWN)
	{
		...
		if (events.button.button == SDL_BUTTON_LEFT)
		{
			...
			p_amo->LoadSur("image//fireball.png");
		...
		else if (events.button.button == SDL_BUTTON_RIGHT)
		{
			...
			p_amo->LoadSur("image//torpedo.png");
